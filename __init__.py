...
def member_json(member='KC87853B', client='HEALTHFIRST', sections=[]):
    ...
    for filepath in sorted(filepaths)[::-1]:
        ...
        file_list = [f.filename for f in zf.filelist if ('Content/' in f.filename) & ('FileMetadata' not in f.filename) & (f.filename.endswith('xml'))]
...
#            if isinstance(v, list): output[k] = [dict(y) for y in set(tuple(x.items()) for x in v)]
        ...
        if time: output['timestamp'] = datetime.strptime(time.group(), '%Y%m%d%H%M%S')
        else:    output['timestamp'] = None
        break
...
def file_processor(filepaths):
    ...
    for filepath in filepaths:
        ...
        if ext not in ['zip', 'xml', 'ZIP', 'XML']:
            ...
            def globit(srchDir):
                ...
                for file in glob.glob(srchDir):
                    if ('Content\\' in file) and ('FileMetadata' not in file) and (file.endswith('xml')): file_list.append(file)
                    globit(file)
            globit(filepath)
        ...
        if output:
            ...
            with open(os.path.join(parent_path, filename+'.json'), 'w') as f: f.write(json.dumps(output, default=_datetime_serializer))

def condition_labeler(member='KC87853B', client='HEALTHFIRST', latest_encounter_date=None):
    assert latest_encounter_date is not None, print("AttributeError: must provide latest_encounter_date as standardized integer type.")
    ...
    condition_mappings = list(pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files',r'codes_ICD_mappings'),"rb")))
    
    client, db = client_mappings[client]
    claims = full_info.fetch_db.claims(member, client, db, with_time=True)
    if not claims: return
    after_30_claims = {k:v for k, v in claims.items() if k > latest_encounter_date and k - latest_encounter_date <= 1440}
    for codes in after_30_claims.values():
        if list(set(codes) & set(condition_mappings)): return 1
    return 0

def readmission(member='KC87853B', client='HEALTHFIRST'):
    
    readmission     = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files',r'readmission'),"rb"))
    client_mappings = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files',r'client_mappings'),"rb"))
    client, db = client_mappings[client]
    ...
    df = df.groupby(['ENC_ID', 'ADM_DATE', 'DSC_DATE'])['CODE'].agg(list).reset_index()
    df['main_1'] = df['CODE'].map(lambda x: True if list(set(x) & set(readmission['inpatient_stays'])) else False)
    df['main_2'] = df['CODE'].map(lambda x: False if list(set(x) & set(readmission['nonacute_inpatient_stays'])) else True)
    df = df.loc[df['main_1'] & df['main_2'], ['ENC_ID', 'ADM_DATE', 'DSC_DATE', 'CODE']]
    if df.shape[0] == 0: return (0, None)
    
    df['exc_1'] = df['CODE'].map(lambda x: False if list(set(x) & set(readmission['pregnancy']+readmission['perinatal']+readmission['chemotherapy']+readmission['rehabilitation']+readmission['transplant'])) else True)
    df['exc_2'] = df['CODE'].map(lambda x: False if list(set(x) & set(readmission['planned_procedure'])) else True)
    df['exc_3'] = df['CODE'].map(lambda x: True if list(set(x) & set(readmission['acute_condition'])) else False)
    df = df.loc[df['exc_1'] & (df['exc_2'] | df['exc_3']), ['ENC_ID', 'ADM_DATE', 'DSC_DATE', 'CODE']]
    if df.shape[0] == 0 or df.shape[0] == 1: return (0, None)
    
    for idx, val in df.iterrows():
        if pd.isnull(val['DSC_DATE']): df.at[idx, 'DSC_DATE'] = val['ADM_DATE']
        if val['DSC_DATE'] == datetime(8888, 12, 31, 0, 0): df.at[idx, 'DSC_DATE'] = val['ADM_DATE']
    df['delta'] = (df['ADM_DATE'].shift(-1) - df['DSC_DATE'].astype('datetime64')).map(lambda x: x.days)
    df = df.loc[(df['delta'] > 0) & (df['delta'] <= 30), :]
    if df.shape[0] == 0: return (0, None)
    else: return (1, _datetime_std(df.at[df.index[-1], 'ADM_DATE']))
    
def member_text(member='KC87853B', client='HEALTHFIRST', sections=[], excludeCode=False, excludeNone=True):
    """
...
def _update_rec(d, u):
    for k, v in u.items():
        if not v: continue
        elif isinstance(v, collections.Mapping): d[k] = _update_rec(d.get(k, {}), v)
...
def _datetime_serializer(o):
    if isinstance(o, (datetime.datetime, datetime.date)): return o.__str__()
