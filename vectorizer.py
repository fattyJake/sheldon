...
class Vectorizer(object):
    ...
    def __init__(self):
        ...
        """
        self.code_system   = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','code_system'),"rb"))
        
        self.all_variables = list(pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','all_variables_new'),"rb")))
        self.punctuation   = '''!"#$%&'*,.:;/<=>?@[\]^_`{|}~•'''
        self.stopwords     = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','stopwords'),"rb"))
        self.bigram        = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','bigram'),"rb"))
        self.vocab         = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','vocab'),"rb"))
        
        self.lab_norm      = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','lab_norm'),"rb"))
        self.vital_norm    = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','vital_norm'),"rb"))
        self.time_norm     = pickle.load(open(os.path.join(os.path.dirname(os.path.realpath(__file__)),r'pickle_files','time_norm'),"rb"))
    ...
    def fit_transform(self, ehr, max_sequence_length, encounter_limit=None):
        ...
        try: latest_ect_time = max([e[0] for e in events if e[1] == 'ECT'])
        except ValueError:
            try: latest_ect_time = self._DT_standardizer(ehr['document']['date'])
        ...
        if T.shape[0] >= max_sequence_length:
            ...
            Q = np.pad(Q, (short_seq_length, 0), 'constant', constant_values=(0, 0))
    ...
    def _add_ngrams(self, l):
        for token in self.bigram[l]:
            if '_' in token: l.append(token)
        return l
    
    # medication vectorizer
    def _medication_vectorizer(self, ehr):
        output = []
        if 'medications' not in ehr.keys(): return output
        for product in ehr['medications']:
            ...
            if not s and e: s = e - 14
            if s and not e: e = s + 14
    ...
    def _allergen_vectorizer(self, ehr):
        output = []
        if 'allergies' not in ehr.keys(): return output
        for allergen in ehr['allergies']:
            ...
            if not e: e = self._DT_standardizer(ehr['document']['date'])
            if not e: continue
            if not s: s = e - 365*24*2
            if not s: continue
            delta = e-s if e >= s else 0
    ...
    def _immuzation_vectorizer(self, ehr):
        output = []
        if 'immunizations' not in ehr.keys(): return output
    ...
    def _lab_vectorizer(self, ehr):
        output = []
        if 'results' not in ehr.keys(): return output
            ...
            for test in results['tests']:
                ...
                if code and system:
                    ...
                    if isinstance(value, str):
                        ...
                        if 'pos' in value.lower(): value = 1
                        elif 'neg' in value.lower(): value = 0
                        # sometimes value and unit together as text format, extract if possible
                        else:
                            value = re.search(r"\-?(\d+|\d+,?)*\.\d+(e\d+)?|\d+", value)
                            if value: value = float(re.sub(r',', '', value.group()))
...
#                    code = test_code+'--'+code
                    ...
                    if isinstance(value, (int, float)): value = np.asscalar(self.lab_norm[code].transform(value))
                    else: value = 0
    ...
    def _encounter_vectorizer(self, ehr):
        output = []
        if 'encounters' not in ehr.keys(): return output
    ...
    def _problem_vectorizer(self, ehr):
        output = []
        if 'problems' not in ehr.keys(): return output
        for problem in ehr['problems']:
            ...
            if code and system:
                ...
                output.append((s, 'PRL', index, np.asscalar(self.time_norm.transform(delta))))
        return list(set(output))
    
    # procedure vectorizer
    def _procedure_vectorizer(self, ehr):
        output = []
        if 'procedures' not in ehr.keys(): return output
    ...
    def _history_vectorizer(self, ehr):
        output = []
        if 'social_history' not in ehr.keys(): return output
    ...
    def _vital_vectorizer(self, ehr):
        output = []
        if 'vitals' not in ehr.keys(): return output
        for results in ehr['vitals']:
            ...
            for test in results['results']:
                ...
                if code and system:
                    ...
                    # z-score normalization
                    if isinstance(value, (int, float)): value = np.asscalar(self.vital_norm[code].transform(value))
                    else: value = 0
    ...
    def _text_vectorizer(self, ehr, latest_ect_time):
        output = []
        texts = []
        if 'chief_complaint' not in ehr.keys(): pass
        else: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(ehr['chief_complaint']['text'])))
        if 'diagnosis' not in ehr.keys(): pass
        else: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(ehr['diagnosis']['text'])))
        if 'history_of_present_illness' not in ehr.keys(): pass
        else: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(ehr['history_of_present_illness']['text'])))
        if 'instructions' not in ehr.keys(): pass
        else:
            for item in ehr['instructions']: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(item['text'])))
        if 'physical_exam' not in ehr.keys(): pass
        else: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(ehr['physical_exam']['text'])))
        if 'system_review' not in ehr.keys(): pass
        else: texts.append(re.sub(r'(\n|[^\x00-\x7F]+)', ' ', str(ehr['system_review']['text'])))
        
        for text in texts:
            text = preprocessing.preprocess(text, negex=True)
            text = re.sub(r"[A-Z]+", '', text)
            text = re.sub(r"(&#39;|<\/?p>|&nbsp;)", '', text)
            date = re.search(r"[0-9]{1,4}(\/|-)[0-9]{1,2}(\/|-)[0-9]{1,4}(\s*T?[0-9]{1,2}(:[0-9]{1,2})+Z?)?", text)
            try: date = self._DT_standardizer(date.group())
            except: date = latest_ect_time
