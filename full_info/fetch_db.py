...
def info(
    ...
    cursor = pyodbc.connect(
        r"DRIVER=SQL Server;" r"SERVER=" + str(db) + ";"
    ).cursor()
    ...
    try:
        output = {
            cursor.description[i][0]: data[0][i]
...
def claims(
    ...
    # initialize
    cursor = pyodbc.connect(
        r"DRIVER=SQL Server;" r"SERVER=" + db + ";"
    ).cursor()
    ...
    cursor.execute(
        ...
        + client
        + """.dbo.tbMember M
                   WHERE mem_ClientMemberID = '"""
        + str(member)
        + """'
                   """
    )
    try:
        member = list(i for i in cursor)[0][0]
    ...
    WHERE mem_id = @Mem_id AND e.enc_ServiceDate BETWEEN '"""
        + date_start
        + """' AND '"""
        + date_end
        + """'
    ...
    WHERE mem_id = @Mem_id AND p.pha_ServiceDate BETWEEN '"""
        + date_start
        + """' AND '"""
        + date_end
        + """'
    ...
    sql = re.sub(
        r" AND (e|p)\.[a-z\_]+ServiceDate BETWEEN 'None' AND 'None'", "", sql
    )
    cursor.execute(sql)
    ...
    if with_time:
        ...
        for i in cursor:
            ...
            if std_dt <= 0:
                std_dt = 0
            if std_dt in output:
                output[std_dt].append(i[3] + "-" + i[4])
            else:
                output[std_dt] = [i[3] + "-" + i[4]]
        for k, v in output.items():
            output[k] = list(set(v))
        return output
    else:
        return list(set([i[3] + "-" + i[4] for i in cursor]))


def readmission_claims(
    member="KC87853B",
    client="CD_HEALTHFIRST",
    db="CARABWDB03",
    date_start=None,
    ...
    FROM """
        + client
        + """.dbo.tbEncounter WITH(NOLOCK)
    WHERE mem_id = @Mem_id AND bltp_Code IS NOT NULL AND enc_ServiceDate BETWEEN '"""
        + date_start
        + """' AND '"""
        + date_end
        + """'
    ...
    INNER JOIN """
        + client
        + """.dbo.tbEncounterRevenue eREV ON e.enc_id = eREV.enc_id
    WHERE mem_id = @Mem_id AND e.enc_ServiceDate BETWEEN '"""
        + date_start
        + """' AND '"""
        + date_end
...
def _ccdID_to_path(packageID):
    ...
    if packageID is None:
        return None
    if isinstance(packageID, (int, str)):
        packageID = [packageID]
    ...
    for table in ["Package", "Package_Archive"]:
        ccds = cursor.execute(
            ...
            FROM [EnterpriseApplications.Queue].[EnterpriseWorkflow].["""
            + table
            + """] WITH(NOLOCK)
            WHERE Id IN ("""
            + ",".join(
                ["'" + str(i) + "'" for i in packageID if i is not None]
            )
            + """)"""
        )
